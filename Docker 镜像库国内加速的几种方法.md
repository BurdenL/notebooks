...
1. 配置国内可用/速度尚可的 Docker Registry Mirrors
   ...
   4. ...
2. 自建 Docker Registry Mirror/Proxy, 并配置为 Mirror
   1. 这里使用 Cloudflare Worker - [cloudflare-docker-proxy](https://link.zhihu.com/?target=https%3A//github.com/ciiiii/cloudflare-docker-proxy) 搭建
...
也可以直接查看 GitHub 仓库：[docker-practice/docker-registry-cn-mirror-test](https://link.zhihu.com/?target=https%3A//github.com/docker-practice/docker-registry-cn-mirror-test/actions) 的 Github Action 执行结果。如最近一次的执行结果为：



![img](https://pic4.zhimg.com/80/v2-ceac439ea02f8b7bd769f8aa2e4238db_720w.webp)



#### 阿里云 Docker 加速服务申请

[阿里云加速器（点击管理控制台 -> 登录账号 -> 右侧镜像工具 -> 镜像加速器 -> 复制加速器地址）](https://link.zhihu.com/?target=https%3A//cr.console.aliyun.com/cn-hangzhou/instances/mirrors)

截图如下：



![img](https://pic3.zhimg.com/80/v2-b5e9a67482c49ff40c3565c337a45c06_720w.webp)
...
这里使用 Cloudflare Worker - [cloudflare-docker-proxy](https://link.zhihu.com/?target=https%3A//github.com/ciiiii/cloudflare-docker-proxy) 搭建。原文 README 步骤有一些问题，可以参考这篇 [README.md](https://link.zhihu.com/?target=https%3A//github.com/east4ming/cloudflare-docker-proxy) 来实施。
...
4. 点击 "Deploy" 按钮部署，部署后如下：
   ...
   ![img](https://pic3.zhimg.com/80/v2-8bfa90e3cee7a68c9825980a9bed52d6_720w.webp)

   

5. 在 Cloudflare 的 DNS 记录里添加 `CNAME` 指向部署后的 `${workername}.${username}.workers.dev` 地址。如下：

   ![img](https://pic1.zhimg.com/80/v2-592e7f41dd0db0edb026ea05f7be3a00_720w.webp)

   

6. 在 Workers 的 HTTP Routes 里，添加 `xxx.your-domain.com/*` 路由指向 cloudflare-docker-proxy, xxx 就是 `docker` `quay` `gcr` 等，如下：

   ![img](https://pic2.zhimg.com/80/v2-7a8ee86c757b701295b7f04575dc0a39_720w.webp)
...
- Docker Hub
- [http://Quay.io](https://link.zhihu.com/?target=http%3A//Quay.io)
- [http://GHCR.io](https://link.zhihu.com/?target=http%3A//GHCR.io) (GitHub 的 Docker registry)
- [http://gcr.io](https://link.zhihu.com/?target=http%3A//gcr.io) (Google Cloud 的 Registry)
- ...
